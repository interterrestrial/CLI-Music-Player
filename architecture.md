...
## 5. System Flow & Sequence Diagram

```mermaid
sequenceDiagram
    autonumber
    actor User
    participant CLI as Terminal (Raw Mode)
    participant Engine as Audio Controller
    participant FS as File System
    participant OS as Audio Subprocess (afplay/vlc)

    User->>CLI: Launch `node musicplayer.js`
    CLI->>FS: readdirSync('./songs')
    FS-->>CLI: Array of .mp3 files
    CLI->>CLI: Render interactive menu (ANSI Escape codes)

    User->>CLI: Press UP / DOWN Arrow
    CLI->>CLI: Update index & re-render in-place (`\x1B[0;0H`)

    User->>CLI: Press ENTER
    CLI->>Engine: playSong()
    Engine->>OS: Extract duration via `afinfo`
    OS-->>Engine: Returns duration (sec)
    Engine->>OS: spawn audio process
    Engine->>CLI: Start timer interval (1s) & draw live seek bar

    User->>CLI: Press 'P' (Pause)
    CLI->>Engine: Pause toggle
    Engine->>OS: Send pause signal / command
    Engine->>Engine: Record pausedAt timestamp & stop progress advance

    User->>CLI: Press 'Q' or Ctrl+C
    CLI->>Engine: Exit trigger
    Engine->>OS: Terminate child processes & clean intervals
    Engine->>CLI: Exit raw mode and quit
```
